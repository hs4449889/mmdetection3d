...
class ScanNetData(object):
    ...
    def __init__(self, root_path, split="train"):
        ...
        split_file = osp.join(
            self.root_dir, "meta_data", f"scannetv2_{split}.txt"
        )
    ...
    def get_intrinsics(self, idx):
        matrix_file = osp.join(
            self.root_dir, "posed_images", idx, "intrinsic.txt"
        )
        mmcv.check_file_exist(matrix_file)
        ...
        def process_single_scene(sample_idx):
            ...
            if not self.test_mode:
                ...
                pts_instance_mask.tofile(
                    osp.join(
                        self.root_dir, "instance_mask", f"{sample_idx}.bin"
                    )
                )
                pts_semantic_mask.tofile(
                    osp.join(
                        self.root_dir, "semantic_mask", f"{sample_idx}.bin"
                    )
                )
