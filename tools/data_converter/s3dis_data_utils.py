...
class S3DISData(object):
    ...
    def __init__(self, root_path, split="Area_1"):
        ...
        self.data_dir = osp.join(
            root_path, "Stanford3dDataset_v1.2_Aligned_Version"
        )
        ...
        self.cat_ids2class = {
            cat_id: i for i, cat_id in enumerate(list(self.cat_ids))
        }
    ...
    def get_infos(self, num_workers=4, has_label=True, sample_id_list=None):
        ...
        def process_single_scene(sample_idx):
            ...
            points.tofile(
                osp.join(
                    self.root_dir, "points", f"{self.split}_{sample_idx}.bin"
                )
            )
            ...
            info["pts_path"] = osp.join(
                "points", f"{self.split}_{sample_idx}.bin"
            )
