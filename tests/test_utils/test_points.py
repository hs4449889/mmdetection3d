...
def test_base_points():
    ...
    assert torch.allclose(
        cat_points.tensor[: len(base_points)], base_points.tensor
    )
    ...
    assert torch.all(
        base_points[:, 3:].tensor == torch.tensor(points_np[:, 3:])
    )
...
def test_cam_points():
    ...
    assert torch.allclose(
        cat_points.tensor[: len(cam_points)], cam_points.tensor
    )
...
def test_lidar_points():
    ...
    assert torch.allclose(
        cat_points.tensor[: len(lidar_points)], lidar_points.tensor
    )
...
def test_depth_points():
    ...
    assert torch.allclose(
        cat_points.tensor[: len(depth_points)], depth_points.tensor
    )
