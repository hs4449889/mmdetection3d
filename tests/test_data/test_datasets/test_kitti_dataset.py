...
def test_getitem():
    ...
    kitti_dataset = KittiDataset(
        data_root, ann_file, split, pts_prefix, pipeline, classes, modality
    )
    ...
    expected_rot_matrix = torch.tensor(
        [
...
def test_evaluate():
    ...
    assert np.isclose(
        ap_dict["KITTI/Overall_3D_AP11_easy"], 3.0303030303030307
    )
    assert np.isclose(
        ap_dict["KITTI/Overall_3D_AP11_moderate"], 3.0303030303030307
    )
    assert np.isclose(
        ap_dict["KITTI/Overall_3D_AP11_hard"], 3.0303030303030307
    )
...
def test_show():
    ...
    eval_pipeline = [
        ...
        dict(
            type="DefaultFormatBundle3D", class_names=classes, with_label=False
        ),
        dict(type="Collect3D", keys=["points"]),
    ]
    tmp_dir = tempfile.TemporaryDirectory()
...
def test_format_results():
    ...
    assert np.allclose(
        result_files[0]["rotation_y"], expected_rotation_y, 1e-3
    )
...
def test_bbox2result_kitti2d():
    ...
    bboxes = np.array(
        [
            [
                [46.1218, -4.6496, -0.9275, 0.5316, 0.5],
