...
def test_seg_show():
    ...
    pts_file_path = osp.join(
        temp_dir, "Area_1_office_2", "Area_1_office_2_points.obj"
    )
    gt_file_path = osp.join(
        temp_dir, "Area_1_office_2", "Area_1_office_2_gt.obj"
    )
    pred_file_path = osp.join(
        temp_dir, "Area_1_office_2", "Area_1_office_2_pred.obj"
    )
...
def test_multi_areas():
    ...
    assert np.all(
        s3dis_dataset.scene_idxs
        == np.array([0, 0, 1, 2, 2, 3, 4, 5, 6, 6, 7, 8, 9, 9, 10, 10, 10])
    )
