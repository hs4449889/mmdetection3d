...
def test_scannet_pipeline():
    ...
    results["ann_info"]["axis_align_matrix"] = info["annos"][
        "axis_align_matrix"
    ]
    ...
    assert torch.allclose(
        gt_bboxes_3d.tensor[:5, :], expected_gt_bboxes_3d, 1e-2
    )
...
def test_scannet_seg_pipeline():
    ...
    scannet_points = np.fromfile(
        osp.join(data_path, info["pts_path"]), dtype=np.float32
    ).reshape((-1, 6))
...
def test_sunrgbd_pipeline():
    ...
    pipelines = [
        ...
        dict(
            type="RandomFlip3D", sync_2d=False, flip_ratio_bev_horizontal=1.0,
        ),
        dict(
            type="GlobalRotScaleTrans",
