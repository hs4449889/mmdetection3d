...
def test_indoor_sample():
    ...
    assert np.all(
        scannet_pts_instance_mask[scannet_choices]
        == scannet_instance_labels_result
    )
    assert np.all(
        scannet_pts_semantic_mask[scannet_choices]
    ...
    expected_repr_str = (
        "PointSample(num_points=5, " "sample_range=None, " "replace=False)"
    )
    assert repr_str == expected_repr_str
    assert np.allclose(
        sunrgbd_point_cloud[sunrgbd_choices], sunrgbd_points_result
    )
...
def test_indoor_seg_sample():
    ...
    scannet_points = np.fromfile(
        "./tests/data/scannet/points/scene0000_00.bin", dtype=np.float32
    ).reshape((-1, 6))
    scannet_results["points"] = DepthPoints(
    ...
    s3dis_points = np.fromfile(
        "./tests/data/s3dis/points/Area_1_office_2.bin", dtype=np.float32
    ).reshape((-1, 6))
