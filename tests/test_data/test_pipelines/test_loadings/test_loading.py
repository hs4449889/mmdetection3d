...
def test_load_points_from_indoor_file():
    ...
    sunrgbd_results["pts_filename"] = osp.join(
        data_path, sunrgbd_info["pts_path"]
    )
    ...
    scannet_results["pts_filename"] = osp.join(
        data_path, scannet_info["pts_path"]
    )
...
def test_load_image_from_file_mono_3d():
    ...
    cam_intrinsic = np.array(
        [[1256.74, 0.0, 792.11], [0.0, 1256.74, 492.78], [0.0, 0.0, 1.0]]
    )
    input_dict = dict(
        img_prefix=None,
    ...
    expected_repr_str = (
        "LoadImageFromFileMono3D(to_float32=False, "
...
def test_point_seg_class_mapping():
    ...
    expected_repr_str = (
        f"PointSegClassMapping(valid_cat_ids={valid_cat_ids}"
        ", max_cat_id=40)"
    )
...
def test_normalize_points_color():
    ...
    assert np.allclose(
        points.color, (color - np.array(color_mean)[None, :]) / 255.0
    )
