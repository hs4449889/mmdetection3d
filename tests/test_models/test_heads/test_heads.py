...
def test_anchor3d_head_loss():
    ...
    losses = self.loss(
        cls_score, bbox_pred, dir_cls_preds, gt_bboxes, gt_labels, input_metas
    )
...
def test_anchor3d_head_getboxes():
    ...
    result_list = self.get_bboxes(
        cls_score, bbox_pred, dir_cls_preds, input_metas
    )
...
def test_parta2_rpnhead_getboxes():
    ...
    cls_score[0] -= 1.5  # too many positive samples may cause cuda oom
    result_list = self.get_bboxes(
        cls_score, bbox_pred, dir_cls_preds, input_metas, proposal_cfg
    )
...
def test_part_aggregation_ROI_head():
    ...
    pred_scores = torch.tensor(
        [0.9722, 0.7910, 0.4690, 0.3300, 0.3345],
        dtype=torch.float32,
        device="cuda",
    )
    pred_labels = torch.tensor(
        [0, 1, 0, 2, 1], dtype=torch.int64, device="cuda"
    )
    ...
    losses = self.forward_train(
        feats_dict, voxels_dict, {}, proposal_list, gt_bboxes_3d, gt_labels_3d
    )
...
def test_point_rcnn_roi_head():
    ...
    losses = self.forward_train(
        rcnn_feats, img_metas, proposal_list, gt_bboxes_3d, gt_labels_3d
    )
    assert losses["loss_cls"] >= 0
    assert losses["loss_bbox"] >= 0
    assert losses["loss_corner"] >= 0
...
def test_h3d_head():
    ...
    aggregated_points = torch.rand(
        [1, num_proposal, 3], dtype=torch.float32
    ).cuda()
    aggregated_features = torch.rand(
        [1, 128, num_proposal], dtype=torch.float32
    ).cuda()
    ...
    size_class_targets = (
        torch.rand([1, num_proposal], dtype=torch.float32).cuda().long()
    )
    size_res_targets = torch.rand(
        [1, num_proposal, 3], dtype=torch.float32
    ).cuda()
    dir_class_targets = (
        torch.rand([1, num_proposal], dtype=torch.float32).cuda().long()
    ...
    objectness_weights = torch.rand(
        [1, num_proposal], dtype=torch.float32
    ).cuda()
    box_loss_weights = torch.rand(
        [1, num_proposal], dtype=torch.float32
    ).cuda()
