...
def test_fp_neck():
    ...
    assert outputs["fp_features"].detach().cpu().numpy().shape == (
        3,
        128,
        16384,
    )
...
def test_lss_view_transformer():
    ...
    rots = torch.tensor(
        [
            [
                [1.0000, 0.0067, 0.0017],
                [-0.0019, 0.0194, 0.9998],
                [0.0067, -0.9998, 0.0194],
            ],
            [
                [0.5480, -0.0104, 0.8364],
                [-0.8360, 0.0250, 0.5481],
                [-0.0266, -0.9996, 0.0050],
            ],
        ]
    ).unsqueeze(0)
    trans = torch.tensor(
        [[-0.0097, 0.4028, -0.3244], [0.4984, 0.3233, -0.3376]]
    ).unsqueeze(0)
    ...
    assert (
        torch.sum((feats_bev - feats_bev_acc).abs() < 0.0001).float()
        / (64 * 128 * 128)
        > 0.99
    )
