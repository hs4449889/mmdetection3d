    ...
    if head.with_mask:
        ...
        if config.mask_roi_extractor:
            ...
            _check_roi_extractor(
                mask_roi_cfg, mask_roi_extractor, bbox_roi_extractor
            )
...
def _check_roi_extractor(config, roi_extractor, prev_roi_extractor=None):
    ...
    if prev_roi_extractor:
        assert (
            roi_extractor.roi_layers[0].aligned
...
def _check_bbox_head(bbox_cfg, bbox_head):
    ...
    if isinstance(bbox_cfg, list):
        ...
        if with_reg:
            out_dim = (
                4 if bbox_cfg.reg_class_agnostic else 4 * bbox_cfg.num_classes
            )
...
def _check_parta2_roi_extractor(config, roi_extractor):
    ...
    assert (
        config.roi_layer.max_pts_per_voxel
...
def _check_parta2_bbox_head(bbox_cfg, bbox_head):
    ...
    if isinstance(bbox_cfg, list):
        ...
        assert (
            bbox_cfg.part_in_channels == bbox_head.part_conv[0][0].in_channels
        )
...
def _check_h3d_bbox_head(bbox_cfg, bbox_head):
    assert bbox_cfg["type"] == bbox_head.__class__.__name__
    assert (
        bbox_cfg.num_proposal * 6
        == bbox_head.surface_center_matcher.num_point[0]
    )
    assert (
        bbox_cfg.num_proposal * 12
        == bbox_head.line_center_matcher.num_point[0]
    )
    assert (
        bbox_cfg.suface_matching_cfg.mlp_channels[-1] * 18
        == bbox_head.bbox_pred[0].in_channels
    )
...
def _check_pointrcnn_roi_extractor(config, roi_extractor):
    assert config["type"] == roi_extractor.__class__.__name__
    assert (
        config.roi_layer.num_sampled_points
