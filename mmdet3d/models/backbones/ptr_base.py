...
class LocalTransformer(nn.Module):
    def __init__(
        ...
        BSC_Encoder = (
            TransformerEncoderLayerPreNorm
            if prenorm
    ...
    def forward(self, xyz, features):
        ...
        group_features, group_xyz = self.grouper(
        ...
        output_features = F.max_pool2d(
            transformed_feats, kernel_size=[1, ns]
        )  # (B, C, npoint)
...
class LocalGlobalTransformer(nn.Module):
    def __init__(
        ...
        BSC_Decoder = (
            TransformerDecoderLayerPreNorm
            if prenorm
    ...
    def forward(self, xyz_tgt, xyz_mem, features_tgt, features_mem):
        ...
        transformed_feats = self.chunk(tgt, mem, memory_mask=mem_mask).permute(
            1, 2, 0
        )
        output_features = self.fc(transformed_feats.unsqueeze(-1)).squeeze(-1)
...
class Pointformer(nn.Module):
    ...
    def __init__(
        ...
        self.FP_modules = nn.ModuleList(
            [
                ...
                PointFPModule(
                    [
                        self.nc_outs[-3] + fp_channels[0][1],
                        fp_channels[1][0],
                        fp_channels[1][1],
                    ],
                    norm_cfg=norm_cfg,
    ...
    def forward(self, points):
        ...
        features = self.feature_conv(
            points[..., 3:].transpose(1, 2).unsqueeze(-1)
        ).squeeze(-1)
        batch, num_points = xyz.shape[:2]
        indices = (
            xyz.new_tensor(range(num_points))
            .unsqueeze(0)
            .repeat(batch, 1)
            .long()
        )
