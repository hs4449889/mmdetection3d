...
class VoteFusion(nn.Module):
    ...
    def forward(self, imgs, bboxes_2d_rescaled, seeds_3d_depth, img_metas):
        ...
        for i, data in enumerate(
            ...
            bbox_2d_origin = bbox_2d_transform(
                img_meta, bbox_2d_rescaled, False
            )
            ...
            if bbox_num == 0:
                ...
                # use zero features
                two_cues = torch.zeros(
                    (15, imvote_num), device=seed_3d_depth.device
                )
                mask_zero = torch.zeros(
                    imvote_num - seed_num, device=seed_3d_depth.device
                ).bool()
                mask_one = torch.ones(
                    seed_num, device=seed_3d_depth.device
                ).bool()
                ...
                ray_angle /= torch.sqrt(
                    torch.sum(ray_angle ** 2, -1) + EPS
                ).unsqueeze(-1)
                ...
                geo_cue = torch.cat([xz, ray_angle], dim=-1).view(
                    seed_num, -1, 5
                )
                ...
                two_cues = (
                    two_cues.reshape(-1, feature_size)
                    .transpose(1, 0)
                    .contiguous()
                )
