...
def multibin_loss(pred_orientations, gt_orientations, num_dir_bins=4):
    ...
    for i in range(num_dir_bins):
        ...
        if valid_mask_i.sum() > 0:
            ...
            pred_offset = F.normalize(
                pred_orientations[valid_mask_i, start:end]
            )
            gt_offset_sin = torch.sin(
                gt_orientations[valid_mask_i, num_dir_bins + i]
            )
            gt_offset_cos = torch.cos(
                gt_orientations[valid_mask_i, num_dir_bins + i]
            )
            reg_loss = F.l1_loss(
