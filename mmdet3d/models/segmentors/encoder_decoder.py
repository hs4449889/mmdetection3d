...
class EncoderDecoder3D(Base3DSegmentor):
    ...
    def _input_generation(
        coords, patch_center, coord_max, feats, use_normalized_coord=False
    ):
    ...
    def _sliding_patch_generation(
        ...
        num_grid_x = int(
            torch.ceil(
                (coord_max[0] - coord_min[0] - block_size) / stride
            ).item()
            + 1
        )
        num_grid_y = int(
            torch.ceil(
                (coord_max[1] - coord_min[1] - block_size) / stride
            ).item()
            + 1
        )
            ...
            for idx_x in range(num_grid_x):
                ...
                if replace:  # duplicate
                    point_idxs_repeat = point_idxs[
                        torch.randint(
                            0, point_idxs.shape[0], size=(num_repeat,)
                        ).to(device)
                    ]
                else:
                    point_idxs_repeat = point_idxs[
    ...
    def slide_inference(self, point, img_meta, rescale):
        ...
        preds = point.new_zeros(
            (point.shape[0], self.num_classes)
    ...
    def simple_test(self, points, img_metas, rescale=True):
        ...
        seg_pred = []
        for point, img_meta in zip(points, img_metas):
            seg_prob = self.inference(point.unsqueeze(0), [img_meta], rescale)[
                0
            ]
