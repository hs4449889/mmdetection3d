...
class LSSViewTransformer(BaseModule):
    ...
    def voxel_pooling_prepare(self, coor, x):
        ...
        coor = (coor - self.grid_lower_bound.to(coor)) / self.grid_interval.to(
            coor
        )
    ...
    def voxel_pooling(self, coor, x):
        ...
        final = torch.zeros(
            (B, C, grid_size[2], grid_size[1], grid_size[0]), device=x.device
        )
    ...
    def voxel_pooling_accelerated(self, x):
        ...
        final = torch.zeros(
            (B, C, gs[2], gs[1], gs[0], self.max_voxel_points), device=x.device
        )
        c = self.coor
