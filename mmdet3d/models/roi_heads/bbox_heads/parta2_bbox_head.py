...
class PartA2BboxHead(BaseModule):
    ...
    def forward(self, seg_feats, part_feats):
        ...
        merged_feature = torch.cat(
            (x_rpn.features, x_part.features), dim=1
        )  # (N, C)
        ...
        cls_score = (
            self.conv_cls(shared_feature)
            .transpose(1, 2)
            .contiguous()
            .squeeze(dim=1)
        )  # (B, 1)
        bbox_pred = (
            self.conv_reg(shared_feature)
            .transpose(1, 2)
            .contiguous()
            .squeeze(dim=1)
        )  # (B, C)
    ...
    def loss(
        ...
        if pos_inds.any() == 0:
            ...
            bbox_weights_flat = (
                ...
                .view(-1, 1)
                .repeat(1, pos_bbox_pred.shape[-1])
            )
    ...
    def multi_class_nms(
        ...
        for k in range(0, self.num_classes):
            ...
            if class_scores_keep.int().sum() > 0:
                original_idxs = class_scores_keep.nonzero(as_tuple=False).view(
                    -1
                )
