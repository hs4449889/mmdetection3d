...
class H3DBboxHead(BaseModule):
    ...
    def forward(self, feats_dict, sample_mod):
        ...
        ret_dict["surface_sem_pred"] = torch.cat(
            (z_semantic, xy_semantic), dim=1
        )
        ...
        surface_features = surface_features.view(
            batch_size, -1, object_proposal
        )
    ...
    def loss(
        ...
        square_dist = self.primitive_center_loss(
    ...
    def get_bboxes(
        self, points, bbox_preds, input_metas, rescale=False, suffix=""
    ):
        ...
        """
        # decode boxes
        obj_scores = F.softmax(bbox_preds["obj_scores" + suffix], dim=-1)[
            ..., -1
        ]
    ...
    def multiclass_nms_single(
        self, obj_scores, sem_scores, bbox, points, input_meta
    ):
        ...
        nonempty_box_inds = torch.nonzero(
            nonempty_box_mask, as_tuple=False
        ).flatten()
        ...
        if self.test_cfg.per_class_proposal:
            ...
            for k in range(sem_scores.shape[-1]):
                ...
                score_selected.append(
                    obj_scores[selected] * sem_scores[selected][:, k]
                )
                labels.append(
    ...
    def get_targets(
        ...
        for index in range(len(gt_labels_3d)):
            if len(gt_labels_3d[index]) == 0:
                ...
                valid_gt_masks.append(
                    gt_labels_3d[index].new_ones(gt_labels_3d[index].shape)
                )
        ...
        line_center_pred = [
            bbox_preds["pred_line_center"][i] for i in range(len(gt_labels_3d))
        ]
        ...
        surface_sem_pred = [
            bbox_preds["surface_sem_pred"][i] for i in range(len(gt_labels_3d))
        ]
    ...
    def get_targets_single(
        ...
        obj_surface_center = obj_surface_center.reshape(-1, 6, 3).transpose(
            0, 1
        )
        ...
        obj_line_center = obj_line_center[:, object_assignment].reshape(
            1, -1, 3
        )
        ...
        objectness_mask_surface = euclidean_dist_line.new_zeros(
            num_proposals * 6
        )
        objectness_label_line = euclidean_dist_line.new_zeros(
            num_proposals * 12, dtype=torch.long
        )
        objectness_mask_line = euclidean_dist_line.new_zeros(
            num_proposals * 12
        )
        ...
        proposal_objectness_mask[
            euclidean_dist1 > self.train_cfg["far_threshold"]
        ] = 1
        ...
        objectness_label_surface[
            ...
            * (
                ...
                < self.train_cfg["mask_surface_threshold"]
            )
        ] = 1
        ...
        cues_match_mask = (
            torch.sum(cues_objectness_label.view(18, num_proposals), dim=0)
            >= 1
        ).float()
