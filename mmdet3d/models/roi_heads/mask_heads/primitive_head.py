...
class PrimitiveHead(BaseModule):
    ...
    def __init__(
        ...
        assert (
            vote_aggregation_cfg["mlp_channels"][0]
    ...
    def forward(self, feats_dict, sample_mod):
        ...
        results[
            "aggregated_indices_" + self.primitive_mode
        ] = aggregated_indices
        ...
        decode_ret = self.primitive_decode_scores(
    ...
    def get_targets(
        ...
        gt_primitive_center = seed_gt_votes.view(
    ...
    def get_targets_single(
        ...
        for i, i_instance in enumerate(instance_labels):
            indices = instance_flag[
                pts_instance_mask[instance_flag] == i_instance
            ]
    ...
    def match_point2plane(self, plane, points):
        ...
        point2plane_dist = torch.abs(
            (points * plane[:3]).sum(dim=1) + plane[-1]
        )
    ...
    def compute_primitive_loss(
        ...
        vote_xyz_reshape = primitive_center.view(
            batch_size * num_proposal, -1, 3
        )
        ...
        if self.primitive_mode != "line":
            ...
            size_loss = self.semantic_reg_loss(
                size_xyz_reshape,
                gt_primitive_semantic,
                dst_weight=gt_primitive_mask.view(
    ...
    def get_primitive_center(self, pred_flag, center):
        ...
        pred_indices = (
            (ind_normal[:, 1, :] > self.surface_thresh).detach().float()
        )
        selected = (
            (ind_normal[:, 1, :] <= self.surface_thresh).detach().float()
        )
    ...
    def _assign_primitive_line_targets(
        ...
        assert (
            len(corners_pair) == len(point2line_matching) == len(center_axises)
        )
        for line_select, center_axis, pair_index in zip(
            ...
            if line_select.sum() > self.train_cfg["num_point_line"]:
                ...
                if with_yaw:
                    line_center = (
                        corners[pair_index[0]] + corners[pair_index[1]]
                    ) / 2
                else:
