...
class BaseShapeHead(BaseModule):
    ...
    def __init__(
        ...
        self.conv_reg = nn.Conv2d(
            out_channels, num_base_anchors * box_code_size, 1
        )

        if use_direction_classifier:
            self.conv_dir_cls = nn.Conv2d(
                out_channels, num_base_anchors * 2, 1
            )
        if init_cfg is None:
            if use_direction_classifier:
                self.init_cfg = dict(
...
class ShapeAwareHead(Anchor3DHead):
    ...
    def _init_layers(self):
        ...
        for task in self.tasks:
            sizes = self.anchor_generator.sizes[
                cls_ptr : cls_ptr + task["num_class"]
            ]
    ...
    def forward_single(self, x):
        ...
        cls_score = torch.cat(
            [result["cls_score"] for result in results], dim=1
        )
        bbox_pred = torch.cat(
            [result["bbox_pred"] for result in results], dim=1
        )
        dir_cls_preds = None
        if self.use_direction_classifier:
    ...
    def get_bboxes_single(
        ...
        mlvl_bboxes_for_nms = xywhr2xyxyr(
            input_meta["box_type_3d"](
                mlvl_bboxes, box_dim=self.box_code_size
            ).bev
        )
