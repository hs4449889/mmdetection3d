...
class AnchorTrainMixin(object):
    ...
    def anchor_target_3d_single(
        ...
        """
        if isinstance(self.bbox_assigner, list) and (
            ...
            for i, assigner in enumerate(self.bbox_assigner):
                ...
                total_bbox_targets.append(
                    bbox_targets.reshape(
                        feat_size, 1, rot_angles, anchors.size(-1)
                    )
                )
            ...
            total_dir_targets = torch.cat(total_dir_targets, dim=-2).reshape(
                -1
            )
            total_dir_weights = torch.cat(total_dir_weights, dim=-2).reshape(
                -1
            )
