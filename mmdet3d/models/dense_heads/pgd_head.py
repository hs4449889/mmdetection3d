...
class PGDHead(FCOSMono3DHead):
    ...
    def get_proj_bbox2d(
        ...
        pos_decoded_bbox2d_preds = distance2bbox(
            pos_points, pos_strided_bbox2d_preds
        )
        ...
        for idx in range(num_imgs):
            ...
            pos_strided_bbox_preds[mask, 2] = pos_bbox_targets_3d.clone()[
                mask, 2
            ]
            ...
            pos_bbox_targets_3d[mask, 6] = (
                torch.atan2(
                    centers2d_targets[:, 0] - cam2img[0, 2], cam2img[0, 0]
                )
                + pos_bbox_targets_3d[mask, 6]
            )
    ...
    def get_pos_predictions(
        ...
        if self.use_depth_classifier:
            flatten_depth_cls_preds = [
                depth_cls_pred.permute(0, 2, 3, 1).reshape(
    ...
    def _get_bboxes_single(
        ...
        for (
            ...
            depth_cls_score = (
                F.softmax(depth_cls_pred, dim=-1)
                .topk(k=2, dim=-1)[0]
                .mean(dim=-1)
            )
            if self.weight_dim != -1:
    ...
    def get_targets(
        ...
        for i in range(num_levels):
            ...
            if self.pred_bbox2d:
                ...
                bbox_targets_3d = torch.cat(
                    [bbox_targets_3d, bbox_targets], dim=1
                )
