...
class MonoFlexHead(AnchorFreeMono3DHead):
    ...
    def forward_single(self, x, input_metas):
        ...
        if self.use_edge_fusion:
            ...
            edge_indices = x.new_zeros(
                (batch_size, max_edge_len, 2), dtype=torch.long
            )
        ...
        for i in range(len(self.group_reg_dims)):
            ...
            for j, conv_reg in enumerate(self.conv_regs[i]):
                ...
                if self.use_edge_fusion and (i, j) in self.edge_fusion_inds:
                    ...
                    out_reg = getattr(
                        self, "edge_fuse_reg_{}_{}".format(i, j)
                    )(
                        reg_feat,
                        out_reg,
                        edge_indices,
    ...
    def decode_heatmap(
        self, cls_score, reg_pred, input_metas, cam2imgs, topk=100, kernel=3
    ):
...
``
        ...
        bboxes2d_target = torch.cat(
            [bboxes2d[:, 0:2] * -1, bboxes2d[:, 2:]], dim=-1
        )
    ...
    def loss(
        ...
        loss_bbox = self.loss_bbox(
            preds["bboxes2d"], target_labels["bboxes2d_target"]
        )
        ...
        loss_offsets2d = self.loss_offsets2d(
            preds["offsets2d"], target_labels["offsets2d_target"]
        )
