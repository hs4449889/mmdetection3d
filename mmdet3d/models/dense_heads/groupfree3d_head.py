...
class GroupFree3DHead(BaseModule):
    ...
    def forward(self, feat_dict, sample_mod):
        ...
        if sample_mod == "fps":
            ...
            sample_inds = torch.topk(points_obj_cls_scores, self.num_proposal)[
                1
            ].int()
    ...
    def loss(
        ...
        for prefix in prefixes:
            ...
            heading_label_one_hot.scatter_(
                2, dir_class_targets.unsqueeze(-1), 1
            )
            dir_res_norm = torch.sum(
                bbox_preds[f"{prefix}dir_res_norm"] * heading_label_one_hot, -1
            )
            ...
            if self.size_cls_agnostic:
                ...
                one_hot_size_targets.scatter_(
                    2, size_class_targets.unsqueeze(-1), 1
                )
    ...
    def get_targets_single(
        ...
        if self.bbox_coder.with_rot:
            ...
            for i in range(gt_labels_3d.shape[0]):
                ...
                indices = torch.nonzero(box_indices, as_tuple=False).squeeze(
                    -1
                )
                ...
                for j in range(self.gt_per_seed):
                    ...
                    vote_targets_tmp[
                        column_indices, j + 3 * self.gt_per_seed
                    ] = i
                    if j == 0:
                        vote_targets_tmp[
            ...
            instance_lable = torch.gather(
                vote_targets, 1, instance_indices
            ).squeeze(-1)
            ...
            for i in torch.unique(pts_instance_mask):
                indices = torch.nonzero(
                    pts_instance_mask == i, as_tuple=False
                ).squeeze(-1)
        ...
        delta_xyz = seed_points.unsqueeze(
            1
        ) - gt_bboxes_3d.gravity_center.unsqueeze(
            0
        )  # (num_seed, gt_num, 3)
        ...
        topk_inds = torch.topk(
            euclidean_dist1, seed_points_obj_topk, largest=False
        )[1] * box_label_mask[:, None] + (box_label_mask[:, None] - 1)
        topk_inds = topk_inds.long()
        topk_inds = topk_inds.view(-1).contiguous()

        sampling_targets = torch.zeros(num_seed + 1, dtype=torch.long).to(
            points.device
        )
    ...
    def get_bboxes(
        self, points, bbox_preds, input_metas, rescale=False, use_nms=True
    ):
        ...
        if self.test_cfg["prediction_stages"] == "last":
            ...
            prefixes = [
                f"s{i}."
                for i in range(
                    self.num_decoder_layers - 3, self.num_decoder_layers
                )
            ]
        else:
            raise NotImplementedError
