...
def show_result(
    ...
    if show:
        ...
        if pred_bboxes is not None:
            if pred_labels is None:
                ...
                palette = (
                    np.random.randint(0, 255, size=(pred_labels.max() + 1, 3))
                    / 256
                )
                labelDict = {}
        ...
        show_path = (
            osp.join(result_path, f"{filename}_online.png")
            if snapshot
            else None
        )
        vis.show(show_path)
    ...
    if gt_bboxes is not None:
        ...
        _write_oriented_bbox(
            gt_bboxes, osp.join(result_path, f"{filename}_gt.obj")
        )
    ...
    if pred_bboxes is not None:
        ...
        _write_oriented_bbox(
            pred_bboxes, osp.join(result_path, f"{filename}_pred.obj")
        )
...
def show_seg_result(
    ...
    if pred_seg is not None:
        ...
        pred_seg_color = np.concatenate(
            [points[:, :3], pred_seg_color], axis=1
        )
    ...
    if pred_seg is not None:
        _write_obj(
            pred_seg_color, osp.join(result_path, f"{filename}_pred.obj")
        )
