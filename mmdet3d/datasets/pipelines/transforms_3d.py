...
class RandomDropPointsColor(object):
    ...
    def __call__(self, input_dict):
        ...
        assert (
            ...
            and "color" in points.attribute_dims
    ...
    def __call__(self, input_dict):
        ...
        if self.sync_2d:
            ...
            if "pcd_horizontal_flip" not in input_dict:
                flip_horizontal = (
                    True if np.random.rand() < self.flip_ratio else False
                )
...
class RandomJitterPoints(object):
    ...
    def __init__(
        self, jitter_std=[0.01, 0.01, 0.01], clip_range=[-0.05, 0.05]
    ):
    ...
    def __call__(self, input_dict):
        ...
        if self.clip_range is not None:
            jitter_noise = np.clip(
                jitter_noise, self.clip_range[0], self.clip_range[1]
            )
    ...
    def __call__(self, input_dict):
        ...
        if sampled_dict is not None:
            ...
            gt_labels_3d = np.concatenate(
                [gt_labels_3d, sampled_gt_labels], axis=0
            )
...
class IndoorPatchPointSample(object):
    ...
    def _patch_points_sampling(self, points, sem_mask):
        ...
        if point_idxs.size >= self.num_points:
            ...
            dup = np.random.choice(
                point_idxs.size, self.num_points - point_idxs.size
            )
            idx_dup = np.concatenate(
                [np.arange(point_idxs.size), np.array(dup)], 0
            )
    ...
    def __call__(self, results):
        ...
        points, choices = self._patch_points_sampling(
...
class BackgroundPointsFilter(object):
    ...
    def __init__(self, bbox_enlarge_range):
        assert (
            is_tuple_of(bbox_enlarge_range, float)
        ...
        self.bbox_enlarge_range = np.array(
            bbox_enlarge_range, dtype=np.float32
        )[np.newaxis, :]
    ...
    def __call__(self, input_dict):
        ...
        valid_masks = ~np.logical_and(
            ~foreground_masks, enlarge_foreground_masks
        )
...
class AffineResize(object):
    ...
    def __call__(self, results):
        ...
        if "centers2d" in results:
            centers2d = self._affine_transform(
                results["centers2d"], trans_affine
            )
            valid_index = (
                (centers2d[:, 0] > 0)
    ...
    def _affine_bboxes(self, results, matrix):
        ...
        for key in results.get("bbox_fields", []):
            ...
            bboxes[:, 2:] = self._affine_transform(bboxes[:, 2:], matrix)
            if self.bbox_clip_border:
                bboxes[:, [0, 2]] = bboxes[:, [0, 2]].clip(
                    0, self.img_scale[0] - 1
                )
                bboxes[:, [1, 3]] = bboxes[:, [1, 3]].clip(
                    0, self.img_scale[1] - 1
                )
            results[key] = bboxes
